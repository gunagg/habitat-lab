...
class DecentralizedDistributedMixin:
    ...
    def init_distributed(self, find_unused_params: bool = True) -> None:
        ...
        self._discr_ddp_hooks = Guard(self.discriminator, self.device)
        self.get_advantages = self._get_advantages_distributed

        self.reducer = self._ddp_hooks.ddp.reducer
        self.discr_reducer = self._discr_ddp_hooks.ddp.reducer
    ...
    def before_backward(self, loss: Tensor) -> None:
        ...
        if self.find_unused_params:
            ...
            self.discr_reducer.prepare_for_backward([loss])
        else:
            self.reducer.prepare_for_backward([])  # type: ignore
            self.discr_reducer.prepare_for_backward([])  # type: ignore
