...
class PPOAgileSeq(nn.Module):
    ...
    def update(self, rollouts: RolloutStorage) -> Tuple[float, float, float]:
        ...
        for _e in range(self.ppo_epoch):
            profiling_wrapper.range_push("PPO.update epoch")
            data_generator = rollouts.recurrent_generator_seq(
                advantages, self.num_mini_batch, self.discr_batch_size, self.discr_rho
            )
            ...
            for sample in data_generator:
                ...
                # Backward pass for GT buffer episodes
                n_envs = recurrent_hidden_states_batch.size(1)
                discr_loss = 0
                rnn_hidden_states = torch.zeros(
                    self.discriminator.net.num_recurrent_layers,
                    n_envs,
                    self.discriminator.net.hidden_size,
                    device=actions_batch.device,
                )

                discr_gt_logits, _ = self.discriminator(
                    discr_observations_batch, rnn_hidden_states, discr_gt_masks_batch, discr_gt_prev_actions_batch
                )
                
                discr_experience_logits, _ = self.discriminator(
                    obs_batch, discr_recurrent_hidden_states_batch, masks_batch, prev_actions_batch
                )

                discr_logits = torch.cat([discr_gt_logits, discr_experience_logits], 0)
                # Targets for GT episodes
